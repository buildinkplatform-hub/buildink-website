...
import { replaceLocale } from "@/i18n/route-utils"
...
export function LocaleSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  function changeLocale(nextLocale: Locale) {
    ...
    window.location.assign(
      `${replaceLocale(pathname, nextLocale)}${query ? `?${query}` : ""}`,
    )
  }
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="border-line text-brand-navy hover:border-primary/40 hover:bg-light-blue focus-visible:ring-primary/20 inline-flex min-h-11 items-center gap-2 rounded-2xl border bg-white px-3.5 text-sm font-semibold transition-all outline-none focus-visible:ring-4"
        >
      ...
      <DropdownMenuContent align="end" className="min-w-56 rounded-[22px] p-2">
        ...
        {locales.map((item) => (
          <DropdownMenuItem key={item} onSelect={() => changeLocale(item)} className="px-3 py-3">
            <span
